...
namespace OMSWeb.Logger
{
    public class Log 
    {
        ...
        public static bool Initialize() 
        {
            ...
            logger = new LoggerConfiguration()
                            ...
                            .WriteTo.Async(writeTo => writeTo.Map("mapInfo", $"0;{DateTime.Now.ToString(DATE_SPLITTER)}"
                                ...
                                , outputTemplate: "{Timestamp:yyyy-MM-dd}#{Timestamp:HH:mm:ss.fff}#{Level}#{Message:lj}{NewLine}{Exception}"
        ...
        public static void FilePrint(LogType type, LogEventLevel level, string format, params object[] args)
        {
            ...
            format = string.Format("{0}#{1}#{2}", (seq++).ToString("X8"), type.ToString(), format);
            logger.ForContext("mapInfo", $"{Convert.ToInt32(type)};{DateTime.Now.ToString(DATE_SPLITTER)}").Write((Serilog.Events.LogEventLevel)level, format);
        }
