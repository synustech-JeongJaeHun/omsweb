...
export const getDurationStr: getDurationStr = (_start, _end) => {
  ...
  if (monthDiff < 2) {
    ...
    return [
      makeDateForms(firstMon),
      makeDateForms(endMon)
    ]
  }

  // 2개월 이상 차이 나는 경우
  const mList = getMonthList(monthDiff)
  return mList.map((mon, idx) => R.cond([
		[R.equals(0), () => makeDateForms([start, endOfMonth(start)])],
		[R.equals(mList.length - 1), () => makeDateForms([startOfMonth(end), end])],
		[R.T, () => {
...
export const getDurationLabel: getDurationLabel = (arr) => {
	...
	const label = isSameStart && isSameEnd ? month: `${month}(${startDay}~${endDay})`
...
export const convertDurationLabel: convertDurationLabel = (label, year) => R.join('_', getStrToStartEnd(label, year))
...
// (220406400) => '7년 1개월 1일'
type convertEpochToStr = (epoch: number, opt?: {
	years?: string
	months?: string
	days?: string
	hours?: string
...
export const convertEpochToStr: convertEpochToStr = (epoch, opt = {
  ...
  const calc = (value) => {
    const func = (timeVariant) => _value => pushAndCalc(_value, timeVariant, calc)
    const gtLte = (gt, lte) => R.allPass([R.gt(timeDic[gt]), R.lte(timeDic[lte])])
