...
function setCameraAndRotation(objective: {
	position?: Position
	viewBoxWidth?: number
	rotation?: number
}) {
	if (objective.position) moveCamera(objective.position)
	if (objective.viewBoxWidth)
		resizeViewBox(
...
function getRotatedPosition(position: Position) {
	...
	const r = Math.hypot(diff.x, diff.y),
		p = Math.atan2(diff.y, diff.x) // in radians

	const rotated =
		(((p * 180) / Math.PI + rotationInfo.value) / 180) * Math.PI // in radians
...
function centerZoom() {
	const baseLength3x = mapSizePropertiesInfo.value.baseLength * 3
	const viewBox =
		cameraTotalInfo.value.viewBoxHeight >
		cameraTotalInfo.value.viewBoxWidth
			? {
					// when display vertical
					width: getWidthFromHeightAndRatio(baseLength3x),
					height: baseLength3x,
			  }
			: {
					// when display horizontal
					width: baseLength3x,
					height: getHeightFromWidthAndRatio(baseLength3x),
			  }
