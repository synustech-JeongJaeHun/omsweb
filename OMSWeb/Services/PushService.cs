    ...
    public class PushService
    {
        ...
        public PushService(IHubContext<OMSHub> hub, CacheService cacheSvc, TrackService trackSvc)
        {
            ...
            this.tableEventMap = new Dictionary<string, DataChangeEventTarget>
            {
                ...
                {"stations", new DataChangeEventTarget(CacheKeys.Stations, new[]{"stationChanged"}, true)},
                {"buffers", new DataChangeEventTarget(CacheKeys.Buffers, new[]{"bufferChanged"}, true)},
                ...
                {"fireshutters", new DataChangeEventTarget(CacheKeys.FireShutters, new[]{"fireShutterMapChanged"}, true)},
                {"fireshutter_status", new DataChangeEventTarget(CacheKeys.FireShutterStatus, new[]{"fireShutterStatusTableChanged"}, true)},
            ...
            this.cacheEventMap = new Dictionary<CacheKeys, string[]>
            {
                ...
                {CacheKeys.FireShutters, new[]{"fireShutterMapChanged"}},
                {CacheKeys.FireShutterStatus, new[]{"fireShutterStatusTableChanged"}},
        ...
        private Task SendDBNotificationAsync(string pushName, DataWatcherPayload payload, object body)
        {
            var meta = new
            {
                ...
                User = payload.User, // only for buffer, station, vehicles, segment_blocking
                Note = payload.Note, // only for buffer, station, vehicles, segment_blocking
