...
const currentRadiusAndQuarterRoundLength = (
  ...
  const radius = (function () {
    if (curveCount === 1) {
      switch (use) {
        case 'Height':
          return Math.min(DefaultRadius, height)
        case 'Width':
          return Math.min(DefaultRadius, width)
        case 'HeightAndWidth':
          return Math.min(DefaultRadius, height, width)
      }
    } else {
      switch (use) {
        case 'Height':
          return Math.min(DefaultRadius, height / 2)
        case 'Width':
          return Math.min(DefaultRadius, width / 2)
        case 'HeightAndWidth':
          return Math.min(DefaultRadius, height / 2, width / 2)
      }
    }
  })()
  const quarterRoundLength = (2 * Math.PI * radius) / 4
...
function makeDFromSegment(
  ...
  const nextCommands = (function () {
    const typePattern = parts
      .map((p) => (p.type === 'D' ? `D` : `E${p.direction}${p.location}`))
      .join(' ')

    const height = Math.abs(endPos.y - startPos.x)
    ...
    switch (typePattern) {
      ...
      case 'D EC4 D EC3 D': {
        const { quarterRoundLength, radius } =
          currentRadiusAndQuarterRoundLength('Width', 2, height, width)
      ...
      case 'D EA2 D EA3 D': {
        const { quarterRoundLength, radius } =
          currentRadiusAndQuarterRoundLength('Height', 2, height, width)
      ...
      case 'D EC2 D EA4 D': {
        ...
        const p2 = {
          x: startPos.x + radius,
          y: startPos.y - startEndLength - radius,
        }
        const p3 = {
          x: endPos.x - radius,
          y: endPos.y + startEndLength + radius,
        }
      ...
      case 'D EA2 D EC4 D': {
        ...
        const p2 = {
          x: startPos.x - startEndLength - radius,
          y: startPos.y + radius,
        }
        const p3 = {
          x: endPos.x + startEndLength + radius,
          y: endPos.y - radius,
        }
