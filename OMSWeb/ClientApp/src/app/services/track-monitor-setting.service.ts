...
type TrackMonitorSetting =
  Record<ColorChangedEvent['key'], ColorChangedEvent['value']>
...
function readTrackSettingFromLocalStorage() {
  try {
    return JSON.parse(localStorage.getItem(TrackSettingLocalStorageKey)) as Partial<TrackMonitorSetting> | null
  } catch {
    return null
  }
}
