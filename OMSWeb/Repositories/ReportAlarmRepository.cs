                    ...
                    var sql = $@"
                        ...
                        (
                            SELECT count(*) from (
                                ...
                                *
                                from vehicle_alarms
                                WHERE time::timestamp BETWEEN hours AND (hours+interval '1 hour')
                                {SubFilter(subsection, value)} {GetSubfilter(subfilter)}
                            ) temp
                        )::int
                        from
        ...
        private string? GetName(string? key)
        {
            switch (key)
            {
                ...
                case "alarm":
                    ...
                    ";
                case "point":
                    return "(SELECT concat(current::text, ' (', p.physical_id ::text, ')')  from points p where concat('p', cast(p.logical_id as varchar)) = current)";
                case "vehicle":
                    return $@"
                        (
                            SELECT logical_id
