            ...
            DateTimeOffset from, DateTimeOffset to, int skip, int take, string condition, string sort, string group)
        {
            ...
            string sql = $@"
                SELECT {Select} FROM (
                        ...
                        INNER JOIN (
                            SELECT history_source_id, max(id) AS max_id,
                                   CASE WHEN max(distance_total) < 0 
               		                    THEN (max(distance_total)/1000000+4294)
                                        ELSE max(distance_total)/1000000
                                   END AS max_dist,
                                   CASE WHEN min(distance_total) < 0 
               		                    THEN (min(distance_total)/1000000+4294)
                                        ELSE min(distance_total)/1000000
                                   END AS min_dist,    
